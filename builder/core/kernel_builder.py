...
class KernelBuilder(BaseModel, IKernelBuilder):
    ...
    def _clean_build(self) -> None:
        ...
        cm.git(self._rcs.paths["AnyKernel3"])
        cm.git(self._rcs.paths["KernelSU"])
    ...
    def _patch_anykernel3(self) -> None:
        ...
        fo.ucopy(
            ...
            self._rcs.paths["AnyKernel3"] / "ramdisk"
        )
        fo.ucopy(
    ...
    def _patch_rtl8812au(self) -> None:
        ...
        makefile = self._rcs.paths[self.codename] /\
                   "drivers" /\
                   "net" /\
                   "wireless" /\
                   "realtek" /\
                   "Makefile"
        kconfig = self._rcs.paths[self.codename] /\
                  "drivers" /\
                  "net" /\
                  "wireless" /\
                  "Kconfig"
        defconfig = self._rcs.paths[self.codename] /\
                    "arch" /\
                    "arm64" /\
                    "configs" /\
                    self._defconfig
    ...
    def _patch_ksu(self) -> None:
        ...
        goback = Path.cwd()
        os.chdir(self._rcs.paths["KernelSU"])
        ...
        os.symlink(
            self._rcs.paths["KernelSU"] / "kernel",
            self._rcs.paths[self.codename] /\
            "drivers" /\
            "kernelsu"
        )
        ...
        target_dir = dcfg.root / "KernelSU" if self._linux_kernel_version == "4.14" else self._rcs.paths[self.codename]
        fo.ucopy(
    ...
    def _linux_kernel_version(self) -> str:
        data = ""
        version = []
        with open(self._rcs.paths[self.codename] / "Makefile", encoding="utf-8") as f:
            data = f.read()
    ...
    def _create_zip(self) -> None:
        ...
        fo.ucopy(
            ...
            "boot" /\
            "Image.gz-dtb",
            self._rcs.paths["AnyKernel3"] / "Image.gz-dtb"
        )
