...
class KernelBuilder(BaseModel, IKernelBuilder):
    ...
    def _rm(self) -> ResourceManager:
        return ResourceManager(codename=self.codename, base=self.base, lkv=self.lkv)
    ...
    def clean_build(self) -> None:
        ...
        cm.git(self._rm.paths[self.codename])
        cm.git(self._rm.paths["AnyKernel3"])
        cm.git(self._rm.paths["KernelSU"])
    ...
    def patch_anykernel3(self) -> None:
        cm.remove(self._rm.paths["AnyKernel3"] / "ramdisk")
        cm.remove(self._rm.paths["AnyKernel3"] / "models")
        fo.ucopy(
            dcfg.root / "builder" / "modifications" / self._ucodename / "anykernel3" / "ramdisk",
            self._rm.paths["AnyKernel3"] / "ramdisk"
        )
        fo.ucopy(
            dcfg.root / "builder" / "modifications" / self._ucodename / "anykernel3" / "anykernel.sh",
            self._rm.paths["AnyKernel3"] / "anykernel.sh"
        )
    ...
    def patch_rtl8812au(self) -> None:
        ...
        makefile = self._rm.paths[self.codename] /\
                   "drivers" /\
                   "net" /\
                   "wireless" /\
                   "realtek" /\
                   "Makefile"
        kconfig = self._rm.paths[self.codename] /\
                  "drivers" /\
                  "net" /\
                  "wireless" /\
                  "Kconfig"
        defconfig = self._rm.paths[self.codename] /\
                    "arch" /\
                    "arm64" /\
                    "configs" /\
                    self._defconfig
    ...
    def patch_ksu(self) -> None:
        ...
        goback = Path.cwd()
        os.chdir(self._rm.paths["KernelSU"])
        ...
        os.symlink(
            self._rm.paths["KernelSU"] / "kernel",
            self._rm.paths[self.codename] /\
            "drivers" /\
            "kernelsu"
        )
        ...
        target_dir = dcfg.root / "KernelSU" if self._linux_kernel_version == "4.14" else self._rm.paths[self.codename]
        fo.ucopy(
    ...
    def patch_qcacld(self) -> None:
        ...
        os.chdir(self._rm.paths[self.codename])
        fo.apply_patch("qcacld_pa.patch")
        os.chdir(goback)

    def patch_ioctl(self) -> None:
        ioctl = self._rm.paths[self.codename] /\
                "drivers" /\
                "platform" /\
                "msm" /\
                "ipa" /\
    ...
    def patch_kernel(self) -> None:
        ...
        clang_cmd = f'{self._rm.paths["clang"] / "bin" / "clang"} --version'
    ...
    def _linux_kernel_version(self) -> str:
        data = ""
        version = []
        with open(self._rm.paths[self.codename] / "Makefile", encoding="utf-8") as f:
            data = f.read()
    ...
    def create_zip(self) -> None:
        ...
        fo.ucopy(
            ...
            "boot" /\
            "Image.gz-dtb",
            self._rm.paths["AnyKernel3"] / "Image.gz-dtb"
        )
        ...
        if not kdir.is_dir():
            os.makedirs(kdir)
        os.chdir(self._rm.paths["AnyKernel3"])
    ...
    def run(self) -> None:
        ...
        self._rm.read_data()
        self._rm.generate_paths()
        self._rm.download()
        self._rm.export_path()
        self.clean_build()
