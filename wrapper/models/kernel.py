...
class KernelBuilder:
    ...
    def __init__(self, codename: str, losversion: str, clean: bool):
    ...
    def _create_vars(self) -> None:
        ...
        tools = ""
        device = ""
        with open(os.path.join(self._workdir, "manifests", "tools.json")) as f:
            tools = json.load(f)
        with open(os.path.join(self._workdir, "manifests", "devices.json")) as f:
            data = json.load(f)
        ...
        for e in self._paths:
            ...
            self._paths[e]["path"] = os.path.join(self._workdir, self._paths[e]["path"])
    ...
    def _modify(self) -> None:
        ...
        #         os.path.join(self._paths[self._codename]["path"],
        #                      "net",
        #                      "mac80211"))
        # ath9k
        ...
        cm.remove(os.path.join(self._paths["AnyKernel3"]["path"], "ramdisk"))
        cm.remove(os.path.join(self._paths["AnyKernel3"]["path"], "models"))
        mcodename = "dumplinger" if self._codename in ["dumpling", "cheeseburger"] else self._codename
        fo.ucopy(os.path.join(self._workdir, "modifications", mcodename, "anykernel3", "ramdisk"),
                 os.path.join(self._paths["AnyKernel3"]["path"], "ramdisk"))
        fo.ucopy(os.path.join(self._workdir, "modifications", mcodename, "anykernel3", "anykernel.sh"),
                 os.path.join(self._paths["AnyKernel3"]["path"], "anykernel.sh"))
        # rtl8812au (apply code patches and remove .git* files)
        msg.note("Adding RTL8812AU drivers into the kernel..")
        fo.ucopy(os.path.join(self._paths["rtl8812au"]["path"]),
                 os.path.join(self._paths[self._codename]["path"],
                              "drivers",
                              "net",
                              "wireless",
                              "realtek",
                              "rtl8812au"))
        os.chdir(os.path.join(self._paths[self._codename]["path"],
                              "drivers",
                              "net",
                              "wireless",
                              "realtek",
        ...
        makefile = os.path.join(self._paths[self._codename]["path"],
                                "drivers",
                                "net",
                                "wireless",
                                "realtek",
                                "Makefile")
        kconfig = os.path.join(self._paths[self._codename]["path"],
                               "drivers",
                               "net",
                               "wireless",
                               "Kconfig")
        defconfig = os.path.join(self._paths[self._codename]["path"],
                                 "arch",
                                 "arm64",
                                 "configs",
    ...
    def _form_release(self, name: str) -> None:
        ...
        fo.ucopy(os.path.join(self._paths[self._codename]["path"],
                              "out",
                              "arch",
                              "arm64",
                              "boot",
                              "Image.gz-dtb"),
                 os.path.join(self._paths["AnyKernel3"]["path"],
                              "Image.gz-dtb"))
        # MD5 hash, kernel version and timestamp
        md5sum = hashlib.md5(os.path.join(self._paths["AnyKernel3"]["path"],
                                          "Image.gz-dtb")
                             .encode('utf-8')).hexdigest()
        with open(os.path.join(self._paths[self._codename]["path"], "Makefile")) as f:
        ...
        kdir = os.path.join(self._workdir, "kernel")
