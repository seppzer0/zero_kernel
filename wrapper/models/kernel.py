...
class KernelBuilder:
    ...
    def _patch_anykernel3(self) -> None:
        ...
        fo.ucopy(
            self._workdir / "modifications" / self._ucodename / "anykernel3" / "ramdisk",
            self._paths["AnyKernel3"]["path"] / "ramdisk"
        )
        fo.ucopy(
            self._workdir / "modifications" / self._ucodename / "anykernel3" / "anykernel.sh",
    ...
    def _patch_kernel(self) -> None:
        ...
        # apply .patch files
        fo.ucopy(
            self._workdir / "modifications" / self._ucodename / "kernel",
            self._paths[self._codename]["path"]
        )
        os.chdir(self._paths[self._codename]["path"])
        for pf in Path.cwd().glob("*.patch"):
            msg.note(f"Applying patch: {pf}")
            ccmd.launch(f"patch -p1 --no-backup-if-mismatch < {pf}")
            os.remove(pf)
        os.chdir(self._workdir)
